...
exports.getThemes = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erreur lors de la récupération des thèmes", error: error.message });
  }
};
...
exports.getSentimentDistribution = async (req, res) => {
  try {
    ...
    const sentimentDistribution = await Event.aggregate([
      { $match: filter },
      {
        $group: {
          _id: "$sentiment",
    ...
    console.error("Erreur dans getSentimentDistribution:", error);
    res.status(500).json({ message: error.message });
  }
};
